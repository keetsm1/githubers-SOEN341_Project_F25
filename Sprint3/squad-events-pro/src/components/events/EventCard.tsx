...
const EventCard: React.FC<EventCardProps> = ({
    ...
    // Realtime attendee count subscription
    React.useEffect(() => {
        let cleanup: (() => void) | undefined;
        (async () => {
            try {
                const { subscribeToEventRegistrationCount } = await import('@/services/database');
                cleanup = subscribeToEventRegistrationCount(eventId, (count) => {
                    setAttendeeCount(count);
                });
            } catch {
                // ignore realtime errors in environments without Supabase
            }
        })();
        return () => {
    ...
    const handleRSVPConfirm = async () => {
        ...
        try {
            const { db } = await import('@/services/database');
            // Prevent RSVPs to past events with a clear message
            if (!isUpcoming) {
                toast({
                    variant: 'destructive',
                    title: 'Event Passed',
            ...
            if (typeof onRSVP === 'function') {
                ...
                if (ticket) {
                    ...
                    toast({
                        ...
                        description: `Your ticket has been generated${(ticket as any)?.id ? ` (#${(ticket as any).id})` : ''}. Check My Tickets to view it.`,
                    });
                    onRefetch?.();
                }
            }
            ...
            if (typeof onRSVP !== 'function') {
                toast({
                    variant: 'destructive',
                    title: 'RSVP Failed',
                    description: e?.message || 'Unable to RSVP at this time',
                });
            }
