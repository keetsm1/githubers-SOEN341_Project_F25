		...
		const run = async () => {
			try {
				...
				try {
					...
					// Respect local payments too so a recent local success isn't overwritten by server checks
					const localPaid = paymentCtx ? paymentCtx.hasPaidLocally(eventId) : false;
					if (mounted) setHasPaid(paid || localPaid);
				} catch {}
			} catch {}
		};
		run();
									...
									{user?.role === 'student' && event.isPaid && (
										<div className="mt-2">
											<PayNowButton
												...
												amount={event.price && event.price > 0 ? event.price : undefined}
												initialPaid={hasPaid}
...
const PayNowButton: React.FC<{ eventId: string; eventTitle?: string; amount?: number; initialPaid?: boolean; onPaid?: () => void }> = ({ eventId, eventTitle, amount, initialPaid = false, onPaid }) => {
	const { toast } = useToast();
	const paymentCtx = usePayment();
	const { markPaid, showConfirmation } = paymentCtx;
	const [loading, setLoading] = React.useState(false);
	const [paid, setPaid] = React.useState<boolean>(() => initialPaid || paymentCtx.hasPaidLocally(eventId));
	...
	const handlePay = async () => {
		...
		try {
			...
			if (res && (String(res.status).toLowerCase() === 'success' || res.status === 'success')) {
				setPaid(true);
				// persist locally
				try { markPaid(eventId, amount, res); } catch {}
				// show confirmation modal with success
				showConfirmation({ eventId, title: eventTitle, amount, status: 'success', retry: null });
				try { if (onPaid) onPaid(); } catch {}
			} else {
				// show failure modal with retry callback
				showConfirmation({ eventId, title: eventTitle, amount, status: 'failure', retry: handlePay });
	...
	// also respect local persistence in render-time check
	if (paid || paymentCtx.hasPaidLocally(eventId)) return <Button variant="primary" disabled>Paid</Button>;
	return (
